...
def resource_manager(TCP_IP='127.0.0.1', TCP_PORT=5011, devices=None):
    if devices is None:
    ...
    while 1: #will loop answering the clients
        ...
        pids_running = [int(pid) for pid in os.listdir('/proc') if pid.isdigit()] #get the list of running processes
        ...
        while 1: #iteracts with this client
            ...
            _, pid = data.decode().split(' ') #split the 'HELLO PID' message
            pid = int(pid)
